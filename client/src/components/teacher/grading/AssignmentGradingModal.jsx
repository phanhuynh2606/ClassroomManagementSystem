...
const AssignmentGradingModal = ({
      ...
      levels: [
        {
          level: "Xuất sắc",
          points: 40,
          description: "Nội dung chính xác, đầy đủ, sâu sắc",
        },
        { level: "Tốt", points: 32, description: "Nội dung đúng, khá đầy đủ" },
        {
          level: "Trung bình",
          points: 24,
          description: "Nội dung cơ bản, một số thiếu sót",
        },
        {
          level: "Yếu",
          points: 16,
          description: "Nội dung sai nhiều hoặc thiếu",
        },
      ],
    },
    {
      ...
      levels: [
        {
          level: "Xuất sắc",
          points: 25,
          description: "Trình bày rõ ràng, logic, ngôn từ chuẩn",
        },
        { level: "Tốt", points: 20, description: "Trình bày tốt, có logic" },
        {
          level: "Trung bình",
          points: 15,
          description: "Trình bày bình thường",
        },
      ...
      levels: [
        {
          level: "Xuất sắc",
          points: 25,
          description: "Tư duy sâu sắc, phân tích tốt",
        },
        ...
        { level: "Trung bình", points: 15, description: "Tư duy cơ bản" },
        {
          level: "Yếu",
          points: 10,
          description: "Thiếu tư duy, không phân tích",
        },
      ],
    },
    {
      ...
      levels: [
        {
          level: "Xuất sắc",
          points: 10,
          description: "Rất sáng tạo, ứng dụng tốt",
        },
  ...
  const handleSave = () => {
    ...
    if (!currentSub?._id) {
      message.error("Submission không hợp lệ!");
      return;
    }
    ...
    form
      ...
      .then((values) => {
        ...
        // Check if grade is valid number (allow 0 as valid grade)
        if (
          gradeValue === null ||
        ...
        if (!values.feedback || values.feedback.trim().length < 5) {
          message.error("Vui lòng nhập nhận xét ít nhất 5 ký tự!");
          return;
        }
        ...
        if (
          currentSub?.gradingHistory &&
          currentSub.gradingHistory.length > 0
        ) {
          changeType = "revision";
        } else if (
          currentSub?.grade !== null &&
          currentSub?.grade !== undefined
        ) {
          changeType = "revision";
        }
  ...
  const handleDownloadFile = async (file) => {
    try {
      ...
      const getFileName = () => {
        ...
        try {
          ...
          if (fileUrl.includes("cloudinary.com") && lastSegment) {
            ...
            const originalExt =
              file.fileType || file.type || file.mimetype || "";
      ...
      try {
        ...
        message.error(
          `Không thể tải xuống file ${fileName}. Vui lòng thử lại sau.`
        );
      }
    }
  };
  ...
  const handleRubricGrade = (criteriaId, level) => {
    ...
    const newTotal = mockRubric.reduce((sum, criteria) => {
      const selectedLevel =
        criteriaId === criteria.id ? level : rubricGrades[criteria.id];
        ...
        <code
          dangerouslySetInnerHTML={{
            __html: content
              ...
              .replace(
                /\b(function|const|let|var|if|else|for|while|return|class|extends|import|export)\b/g,
  ...
  const handleNavigateSubmission = (direction) => {
    ...
    if (direction === "prev" && currentSubmissionIndex > 0) {
      const validIndex = findNextValidSubmission(
  ...
  const getCurrentSubmission = () => {
    if (
      ...
      currentSubmissionIndex < allSubmissions.length
    ) {
  ...
  const detectContentType = (content) => {
    ...
    const codeScore = codeIndicators.reduce((score, indicator) => {
      return (
        score +
        (content.toLowerCase().includes(indicator.toLowerCase()) ? 1 : 0)
      );
    }, 0);
        ...
        <div>
          <div className="flex justify-between items-center">
            <Space>
              <TrophyOutlined />
              Chấm điểm bài tập:{" "}
              {String(assignment?.title || "Unknown Assignment")}
            </Space>
            {(() => {
              ...
              const currentValidIndex = validSubmissions.findIndex(
              ...
              return (
                validSubmissions.length > 1 && (
                  <Space className="mr-6">
                    ...
                    <Text strong>
                      {Math.max(currentValidIndex + 1, 1)} /{" "}
                      {validSubmissions.length}
                    </Text>
                    <Button
                      size="small"
                      disabled={
                        currentValidIndex >= validSubmissions.length - 1
                      }
            ...
            <Space>
              <Text>
                ...
                {typeof currentSub?.student === "object"
                  ? currentSub.student.fullName ||
                    currentSub.student.name ||
                    "Unknown Student"
              ...
              {currentSub?.grade !== null &&
                currentSub?.grade !== undefined && (
                  <Tag color="blue">
                    Điểm: {Number(currentSub.grade || 0)}/
                    {Number(maxGrade || 100)}
                  </Tag>
                )}
            </Space>
        ...
        <Button
          ...
          title={
            ...
            currentSub?.status === "missing"
              ...
              ? "Vui lòng nhập nhận xét ít nhất 5 ký tự"
                  ...
                  children: (
                    <>
                      ...
                      {currentSub?.submittedAt ? (
                        <>
                          <Text>
                            {moment(currentSub.submittedAt).format(
                              "DD/MM/YYYY HH:mm"
                            )}
                          </Text>
                          {latePenalty > 0 && (
          ...
          <Card title="📊 Điểm số" size="small">
            ...
            {Object.keys(rubricGrades).length > 0 && (
              <div className="mt-3">
                <Divider />
                <Text strong>
                  Rubric: {calculateRubricTotal()}/
                  {mockRubric.reduce((sum, r) => sum + r.maxPoints, 0)}
                </Text>
              </div>
            )}
          </Card>
                  ...
                  <Card
                   ...
                   >
                     ...
                     {(currentSub?._id?.toString().startsWith("missing_") ||
                       ...
                       <div className="text-center py-8">
                         ...
                         <Text type="secondary" className="text-lg">
                           Học sinh này chưa submit bài assignment trong hệ thống
                         </Text>
                         <div className="mt-4 p-4 bg-yellow-50 rounded-lg border border-yellow-200">
                           <Text strong>Thông tin:</Text>
                           <ul className="mt-2 text-left">
                             <li>📅 Hạn nộp: {moment(assignment?.dueDate).format("DD/MM/YYYY HH:mm")}</li>
                             <li>⏰ Hiện tại: {moment().format("DD/MM/YYYY HH:mm")}</li>
                             <li>📊 Trạng thái: Chưa có submission</li>
                           </ul>
                         </div>
                       </div>
                     )}

                     {/* Text Submission - only show if actually submitted */}
                     {currentSub?.content && 
                      !currentSub?._id?.toString().startsWith("missing_") &&
                      currentSub?.status !== "missing" &&
                      currentSub?.submittedAt && (
                      <div className="mb-4">
                        ...
                        <Card size="small" className="bg-gray-50">
                          {renderContent(String(currentSub.content || ""))}
                                         ...
                                         {/* File Attachments - only show if actually submitted */}
                    ...
                    {(currentSub?._id?.toString().startsWith("missing_") ||
                      currentSub?.status === "missing") && (
                      <Alert
                        message="⚠️ Học sinh chưa nộp bài assignment"
                        description={
                          <div>
                            <p>
                              <strong>
                                Học sinh này chưa submit bài assignment trong hệ
                                thống.
                              </strong>
                            </p>
                            <p>Các hành động có thể thực hiện:</p>
                            <ul className="mt-2 ml-4">
                              <li>📧 Gửi email nhắc nhở học sinh nộp bài</li>
                              <li>📞 Liên hệ trực tiếp với học sinh</li>
                              <li>
                                ⏰ Gia hạn thời gian nộp bài nếu có lý do chính
                                đáng
                              </li>
                              <li>
                                📊 Sử dụng chức năng Auto-Grade để tự động cho
                                điểm 0
                              </li>
                            </ul>
                          </div>
                    ...
                    {currentSub?.status !== "missing" &&
                      ...
                      (!currentSub?.attachments ||
                        currentSub.attachments.length === 0) && (
                        <Alert
                          message="📝 Bài nộp trống"
                          description={
                            <div>
                              <p>
                                <strong>
                                  Học sinh đã nộp bài nhưng không có nội dung.
                                </strong>
                              </p>
                              <p>
                                Thời gian nộp:{" "}
                              ...
                              <p>Có thể học sinh đã:</p>
                              <ul className="mt-2 ml-4">
                                <li>❌ Quên đính kèm file</li>
                                <li>❌ Quên nhập nội dung text</li>
                                <li>⚠️ Gặp lỗi kỹ thuật khi submit</li>
                              </ul>
                              <p className="mt-2">
                                <em>Nên liên hệ học sinh để kiểm tra.</em>
                              </p>
                            </div>
                          }
                          type="warning"
                ...
                children: (
                  <Card size="small">
                    <div className="mb-4 flex justify-between items-center">
                      <div>
                        <Text strong>
                          Tổng điểm rubric: {calculateRubricTotal()}/
                          {mockRubric.reduce((sum, r) => sum + r.maxPoints, 0)}
                        </Text>
                        <Button
                          size="small"
                        ...
                        label: (
                          <div className="flex justify-between items-center">
                            ...
                            {rubricGrades[criteria.id] && (
                              <Tag color="blue">
                                {rubricGrades[criteria.id].points}/
                                {criteria.maxPoints}
                              </Tag>
                            )}
                          </div>
                              ...
                              {criteria.levels.map((level) => (
                                <Card
                                  ...
                                  className={`cursor-pointer ${
                                    rubricGrades[criteria.id]?.level ===
                                    level.level
                                      ? "border-blue-500 bg-blue-50"
                                      : "hover:border-gray-400"
                                  }`}
                                  onClick={() =>
                                    handleRubricGrade(criteria.id, level)
                                  }
                                >
                                  <div className="flex justify-between items-center">
                                    <div>
                  ...
                  <Card size="small">
                    ...
                    >
                      <Row gutter={16}>
                          ...
                          <Form.Item
                            ...
                            rules={[
                              ...
                              },
                              {
                                ...
                                min: 0,
                                max: maxGrade,
                                message: `Điểm phải từ 0 đến ${maxGrade}!`,
                              },
                              {
                                validator: (_, value) => {
                                  if (
                                    ...
                                    return Promise.reject(
                                      new Error("Điểm phải là số hợp lệ!")
                                    );
                                  }
                                  if (value < 0) {
                                    return Promise.reject(
                                  ...
                                  if (value > maxGrade) {
                                    return Promise.reject(
                                      new Error(
                                        `Điểm không được vượt quá ${maxGrade}!`
                                      )
                                    );
                                  }
                        ...
                        <Col span={8}>
                          <div className="mb-2">
                            ...
                            <Text
                              type={
                                grade >= maxGrade * 0.7 ? "success" : "danger"
                              }
                              className="font-bold"
                            >
                          ...
                          },
                          {
                            validator: (_, value) => {
                              if (!value || value.trim().length === 0) {
                                return Promise.reject(
                                  new Error("Nhận xét không được để trống!")
                                );
                              }
                              if (value.trim().length < 5) {
                                return Promise.reject(
                                  new Error(
                                    "Nhận xét quá ngắn, cần ít nhất 5 ký tự!"
                                  )
                                );
                              }
                              return Promise.resolve();
                      ...
                      >
                        <TextArea
                          rows={8}
                          placeholder="Nhập nhận xét chi tiết về bài làm của học sinh... (ít nhất 5 ký tự)"
                          showCount
                          maxLength={2000}
                          value={feedback}
                          onChange={(e) => {
                      ...
                      <div className="mb-3">
                        ...
                        <Space wrap>
                          <Button
                            ...
                            onClick={() => {
                              const template =
                                "Bài làm tốt, nội dung đầy đủ và chính xác. Trình bày rõ ràng, logic. Cần chú ý thêm về chính tả và ngôn từ.";
                          ...
                          <Button
                            ...
                            onClick={() => {
                              const template =
                                "Bài làm đạt yêu cầu cơ bản. Nội dung đúng nhưng chưa sâu sắc. Nên bổ sung thêm ví dụ và phân tích chi tiết hơn.";
                              setFeedback(template);
                              form.setFieldsValue({ feedback: template });
                            }}
                          >
                          ...
                          <Button
                            ...
                            onClick={() => {
                              const template =
                                "Bài viết hay, thể hiện tư duy tốt và kiến thức vững. Cấu trúc logic, ngôn từ phù hợp. Tiếp tục phát huy!";
                              setFeedback(template);
                              form.setFieldsValue({ feedback: template });
                            }}
                          >
                          ...
                          <Button
                            ...
                            onClick={() => {
                              const template =
                                "Bài làm chưa đạt yêu cầu. Thiếu nội dung quan trọng, trình bày chưa rõ ràng. Cần học lại và làm bài mới.";
                              setFeedback(template);
                              form.setFieldsValue({ feedback: template });
                            }}
                          >
                          ...
                          <Button
                            ...
                            onClick={() => {
                              const template =
                                "Bài làm xuất sắc! Nội dung sâu sắc, trình bày logic, có tư duy phản biện. Đây là bài mẫu cho cả lớp!";
                              setFeedback(template);
                              form.setFieldsValue({ feedback: template });
                            }}
                          >
                          ...
                          <Button
                            ...
                            onClick={() => {
                              const template =
                                "File đính kèm rõ nét, đầy đủ. Nội dung được trình bày khoa học. Cần bổ sung thêm phần kết luận.";
                              setFeedback(template);
                              form.setFieldsValue({ feedback: template });
                            }}
                          >
                    ...
                    <Timeline
                      items={(() => {
                        ...
                        if (currentSub?.submittedAt) {
                          const isLate =
                            assignment?.dueDate &&
                            moment(currentSub.submittedAt).isAfter(
                            ...
                            children: (
                              <>
                                ...
                                {isLate && assignment?.dueDate && (
                                  <div className="mt-1">
                                    <Tag size="small" color="warning">
                                      ⚠️ Muộn{" "}
                                      {moment(currentSub.submittedAt).diff(
                                        moment(assignment.dueDate),
                                        "hours"
                                      )}{" "}
                                      giờ
                                ...
                                {currentSub?.attachments?.length > 0 && (
                                  <div className="mt-1">
                                    <Text type="secondary">
                                      📎 {currentSub.attachments.length} file
                                      đính kèm
                                    </Text>
                                  </div>
                                )}
                        ...
                        // Thêm grading history events (SUBMISSION level)
                        if (
                          currentSub?.gradingHistory &&
                          currentSub.gradingHistory.length > 0
                        ) {
                          currentSub.gradingHistory.forEach(
                            (grading, index) => {
                              const isAutoGrade =
                                grading.changeType === "auto-grade" ||
                                grading.gradedByName === "System Auto-Grade";
                                  ...
                                  <>
                                    <Text strong>
                                      {isAutoGrade
                                        ...
                                        : "👨‍🏫 Manual Grade"}{" "}
                                      (
                                      {grading.changeType === "initial"
                                        ? "Lần đầu"
                                        : grading.changeType === "revision"
                                        ? "Sửa điểm"
                                        : grading.changeType === "appeal"
                                        ? "Phúc khảo"
                                        : grading.changeType === "correction"
                                        ? "Chỉnh sửa"
                                        : grading.changeType === "auto-grade"
                                        ? "Tự động"
                                        : "Cập nhật"}
                                      ...
                                      {grading.latePenalty?.applied && (
                                        ...
                                        >
                                          {" "}
                                          (Điểm : {grading.originalGrade} -
                                          Penalty: -{" "}
                                          {grading.latePenalty.percentage}%)
                                        </span>
                                      )}
                                    </Text>
                                    <br />
                                    <Text type="secondary">
                                      {moment(grading.gradedAt).format(
                                        "DD/MM/YYYY HH:mm"
                                      )}{" "}
                                      -{" "}
                                      {isAutoGrade
                                        ...
                                        : grading.gradedByName ||
                                          grading.gradedBy?.fullName ||
                                          grading.gradedBy?.name ||
                                          (typeof grading.gradedBy === "object"
                                            ? "Giáo viên"
                                            : grading.gradedBy) ||
                                          "Giáo viên"}
                                    </Text>
                                    ...
                                    {grading.rubricGrades &&
                                      Object.keys(grading.rubricGrades).length >
                                        0 && (
                                        <div className="mt-1">
                                          <Text type="secondary">
                                    ...
                                    {grading.previousGrade !== null &&
                                      grading.previousGrade !== undefined && (
                                        <div className="mt-1">
                                          <Tag
                                            size="small"
                                            color={
                                              grading.grade >
                                              grading.previousGrade
                                                ? "green"
                                                : "orange"
                                            }
                                          >
                                    ...
                                    {isAutoGrade && grading.gradeReason && (
                                      <div className="mt-1">
                        ...
                        // Thêm assignment-level auto-grade context (ASSIGNMENT level)
                        // if (assignment?.autoGradeHistory && assignment.autoGradeHistory.length > 0) {
                        //   assignment.autoGradeHistory.forEach((autoGrade) => {
                        //     // Chỉ hiển thị nếu current student có trong batch auto-grade này
                        //     const currentStudentId = typeof currentSub?.student === 'object'
                        //       ? currentSub.student._id || currentSub.student.id
                        //       : currentSub?.student;

                        //     const wasAutoGraded = autoGrade.studentsAutoGraded &&
                        //       autoGrade.studentsAutoGraded.some(studentId =>
                        //         studentId.toString() === currentStudentId?.toString()
                        //       );

                        //     if (wasAutoGraded) {
                        //       events.push({
                        //         timestamp: new Date(autoGrade.autoGradedAt),
                        //         type: 'batch-auto-grade',
                        //         children: (
                        //           <>
                        //             <Text strong>
                        //               🎯 Batch Auto-Grade Event: {autoGrade.gradeValue} điểm
                        //             </Text><br />
                        //             <Text type="secondary">
                        //               {moment(autoGrade.autoGradedAt).format('DD/MM/YYYY HH:mm')} -
                        //               Scheduler tự động chấm {autoGrade.studentsAutoGraded.length} học sinh
                        //             </Text>
                        //             {autoGrade.reason && (
                        //               <div className="mt-1">
                        ...
                        //                 📊 Batch Operation: {autoGrade.studentsAutoGraded.length} students
                        //               </Tag>
                        //             </div>
                        //           </>
                        //         )
                          ...
                          events.push({
                            ...
                            children: (
                              <>
                                ...
                                <Text type="secondary">
                                  {currentSub.gradedAt
                                    ? moment(currentSub.gradedAt).format(
                                        "DD/MM/YYYY HH:mm"
                                      )
                                ...
                                {currentSub.feedback && (
                                  <div className="mt-1">
                                    <Text type="secondary" className="italic">
                                      "💭{" "}
                                      {String(currentSub.feedback || "")
                                        .length > 50
                                        ? String(currentSub.feedback).substring(
                                            0,
                                            50
                                          ) + "..."
                                        : String(currentSub.feedback || "")}
